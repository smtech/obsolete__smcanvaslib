...
if(!isset($argc)) {
	define('APP_URL', (isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['SERVER_NAME'] . preg_replace('|(.*)/' . basename(__DIR__) . '|', '$1', str_replace(realpath($_SERVER['DOCUMENT_ROOT']), '', realpath(__DIR__))));
